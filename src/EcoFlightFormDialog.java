 ...
 * as input form to input and edit Economy Flight's details
...
public class EcoFlightFormDialog extends JDialog
		implements ActionListener
{
	// attributes
	private EconomyFlight ecoFlight;
	private boolean valid;

	// dialog attributes
	JTextField ori, dest, date, dep, arr, price, childPerc;
	private JButton okBtn, resetBtn, cancelBtn;
	private JLabel errInd1, errInd2, errInd3,
			errInd4, errInd5, errInd6, errInd7;
	private JLabel errText1, errText2, errText3, errText4,
			errText5, errText6, errText7;
	...
	public EcoFlightFormDialog(JFrame parentMenu)
	{	
		// attributes
		...
		JLabel titleLbl = new JLabel("Economy Flight Details Form");
	...
	public void actionPerformed(ActionEvent ae)
	{
		if (ae.getSource() == okBtn)
		{
			...
			// pass validation & create new economy flight
			if (valid && validDate && validPrc && validChl &&
				ecoFlight == null) {
				ecoFlight = new EconomyFlight(oriVal,
					destVal, dateObj, depVal+" - "+arrVal,
					Double.parseDouble(prcVal),
					Integer.parseInt(chlPercVal));
				setVisible(false);
			
			// pass validation & edit economy flight
			} else if (valid && validDate && validPrc && validChl &&
				ecoFlight != null) {
				double ecoPrice = Double.parseDouble(prcVal);
				int ecoChlPerc = Integer.parseInt(chlPercVal);
				ecoFlight.setOrigin(oriVal);
				ecoFlight.setDestination(destVal);
				ecoFlight.setDate(dateObj);
				ecoFlight.setDeparr(depVal+" - "+arrVal);
				ecoFlight.setPrice(ecoPrice);
				ecoFlight.setChildPerc(ecoChlPerc);
				ecoFlight.setAdultPrice(
						ecoFlight.calcAdultPrice(ecoPrice));
				ecoFlight.setChildPrice(
						ecoFlight.calcChildPrice(ecoPrice, ecoChlPerc));
				setVisible(false);
	...
	// getter setter
	public EconomyFlight getFlight() {return ecoFlight;}
	public void setFlight(EconomyFlight flight) {ecoFlight = flight;}
}